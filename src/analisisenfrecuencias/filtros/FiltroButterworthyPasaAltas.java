 ...
 * @author yohov
 */
public class FiltroButterworthyPasaAltas extends FiltroFrecuencia{
	private int radio;
	private Dimension dim;
	private int n;

	public FiltroButterworthyPasaAltas(int radio, Dimension dim, int n) {
	...
	public Image crearFiltro() {
		...
		for(int i=0; i < bi.getWidth();i++){
			for(int j=0; j < bi.getHeight();j++){
				...
				double valor = 1-(1/(1+Math.pow((r/this.radio),2*this.n)));
