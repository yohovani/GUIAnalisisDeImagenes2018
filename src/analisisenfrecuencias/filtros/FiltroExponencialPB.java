 ...
 * @author yohov
 */
public class FiltroExponencialPB extends FiltroFrecuencia{
	private int radio;
	private Dimension dim;
	private int n;

	public FiltroExponencialPB(int radio, Dimension dim, int n) {
	...
	public Image crearFiltro() {
		...
		for(int i=0; i < bi.getWidth();i++){
			for(int j=0; j < bi.getHeight();j++){
				...
				double valor = Math.pow(Math.E, -Math.pow((r/radio), n));
